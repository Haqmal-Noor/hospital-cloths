...
router.get('/:id', authenticateToken, async (req, res) => {
  try {
    ...
    const isAuthorized = req.user.role === 'admin' ||
      ...
      (req.user.role === 'visitor' && order.visitorId && order.visitorId._id.toString() === req.user._id.toString()) ||
      (req.user.role === 'tailor' && order.tailorId && order.tailorId._id.toString() === req.user._id.toString());
...
// Create new order
router.post('/', authenticateToken, authorizeRoles('visitor', 'customer'), async (req, res) => {
  try {
    const orderData = {
      ...req.body,
...
router.put('/:id/assign-tailor', authenticateToken, authorizeRoles('admin'), async (req, res) => {
  try {
...
router.put('/:id/status', authenticateToken, async (req, res) => {
  try {
    ...
    const canUpdateStatus = req.user.role === 'admin' ||
      (req.user.role === 'tailor' && order.tailorId && order.tailorId.toString() === req.user._id.toString());
